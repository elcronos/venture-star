...
import {
  TEST_SEED,
  startCampaignFromHome,
  tick,
  waitForGameHook,
} from './helpers';

test('T-E2E-V0-001: home setup pins the campaign seed and launches the real dock screen', async ({
  page,
}) => {
  await page.goto('/');
  ...
  await expect(
    page.getByRole('heading', { name: 'Hearthlight' }),
  ).toBeVisible();
  ...
  expect(
    state.ships.find((ship) => ship.id === state.playerShipId)!.dockedPlanetId,
  ).toBe('planet-000');
});

test('LOOP-PAUSE-001: launch, pause, and resume do not advance simulation while paused', async ({
  page,
}) => {
  ...
  await tick(page, 120);
  expect(await page.evaluate(() => window.__GAME__!.state!.tick)).toBe(
    pausedAt,
  );

  await page.getByRole('button', { name: 'Resume', exact: true }).click();
  await page.evaluate(() =>
    window.__GAME__!.input({ type: 'pause', paused: true }),
  );
  await tick(page, 1);
  expect(await page.evaluate(() => window.__GAME__!.state!.running)).toBe(
    false,
  );
  await expect(
    page.getByRole('heading', { name: 'Flight — Venture Star' }),
...
test('Galaxy map navigation is paused, wrap-aware, and starts a route through the UI', async ({
  page,
}) => {
  await startCampaignFromHome(page);
  await page
  ...
  await expect(
    page.getByRole('grid', { name: 'Galaxy map, 10 by 10 sectors' }),
  ).toBeVisible();
  await page.screenshot({
    path: `test-results/galaxy-${test.info().project.name}.png`,
    fullPage: true,
  });
  expect(await page.evaluate(() => window.__GAME__!.state!.running)).toBe(
    false,
  ...
  await expect(
    page.getByRole('heading', { name: 'Route forecast' }),
  ).toBeVisible();
  await page.screenshot({
    path: `test-results/galaxy-route-${test.info().project.name}.png`,
    fullPage: true,
  });
  await expect(
    page.getByText('Normal fuel · direct-route cruise estimate'),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Start autopilot' }).click();
  await expect(
    page.getByRole('heading', { name: 'Flight — Venture Star' }),
  ).toBeVisible();
  // The status line names the phase and the destination, not a static label.
  await expect(
    page.getByText(/(Travelling|Braking|Arrived): Sector 1,1/),
  ).toBeVisible();
});
