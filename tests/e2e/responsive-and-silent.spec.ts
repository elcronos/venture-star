...
async function audioFiles(root: string): Promise<string[]> {
  ...
  for (const entry of await readdir(root, { withFileTypes: true })) {
    ...
    else if (AUDIO_EXTENSIONS.has(entry.name.slice(entry.name.lastIndexOf(".")).toLowerCase())) found.push(path);
  }
  return found;
}
...
test.beforeEach(async ({ page }) => {
  await page.addInitScript(() => {
    ...
    for (const key of ["Audio", "AudioContext", "webkitAudioContext", "OfflineAudioContext", "webkitOfflineAudioContext"] as const) {
...
test("T-F-10: the app ships and runs without audio files, elements, requests, or runtime construction", async ({ page }) => {
  const requestedAudio: string[] = [];
  page.on("request", (request) => {
    if (/\.(aac|flac|m4a|mp3|ogg|opus|wav|weba)(?:$|\?)/i.test(request.url())) requestedAudio.push(request.url());
  });
  await page.goto("/");

  expect(await audioFiles(process.cwd())).toEqual([]);
  expect(await page.locator("audio, source[type^='audio/']").count()).toBe(0);
  expect(await page.evaluate(() => (window as any).__AUDIO_CONSTRUCTIONS__)).toEqual([]);
  expect(requestedAudio).toEqual([]);
});

test("desktop and mobile render the same campaign actions without viewport overflow", async ({ page }, testInfo) => {
  await startCampaignFromHome(page);
  const primaryLaunch = page.getByRole("button", { name: "Launch", exact: true }).first();
  ...
  const touchControls = page.getByRole("application", { name: "Analog flight joystick" });
  if (testInfo.project.name === "mobile") {
    await expect(touchControls).toBeVisible();
    const controls = page.getByRole("group", { name: "Discrete flight controls" }).getByRole("button");
