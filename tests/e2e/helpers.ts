...
export async function startCampaignFromHome(
  page: Page,
  seed = TEST_SEED,
): Promise<void> {
  await page.goto('/');
...
export async function flyToEntity(page: Page, entityId: string): Promise<void> {
  await page.evaluate((targetId) => {
    ...
    const signedHeadingDelta = (from: number, to: number) =>
      ((to - from + 98_304) % 65_536) - 32_768;
    ...
    for (let attempts = 0; attempts < 4_000; attempts++) {
      ...
      const desired =
        ((Math.round((Math.atan2(dy, dx) / (Math.PI * 2)) * 65_536) % 65_536) +
          65_536) %
        65_536;
      ...
      const turn =
        Math.abs(headingError) <= 350 ? 0 : headingError < 0 ? -1 : 1;
      const throttle = shouldBrake
        ? 0
        : Math.abs(headingError) > 1_400
          ? 0
          : distance > 240
            ? 0.65
            : 0.25;
      game.input({
        ...
        throttle,
        turn,
        ...(shouldBrake ? { brake: true } : {}),
      });
      game.tick(1);
    }
    throw new Error(
      `Failed to reach ${targetId} within deterministic tick budget`,
    );
  }, entityId);
}
