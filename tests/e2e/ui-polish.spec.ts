...
test('UX-T-023 setup explains the campaign and offers the full preferred galaxy', async ({
  page,
}) => {
  await page.goto('/');
  ...
  await expect(
    page.getByText(/destruction ends the campaign permanently/),
  ).toBeVisible();
  ...
  if ((page.viewportSize()?.width ?? 0) >= 1024) {
    const launch = await page
      .getByRole('button', { name: 'Launch campaign' })
      .boundingBox();
    expect(launch!.y + launch!.height).toBeLessThanOrEqual(
      page.viewportSize()!.height,
    );
  }
});

test('UX-T-060 live HUD updates preserve captured flight controls', async ({
  page,
}, testInfo) => {
  test.skip(
    !testInfo.project.name.includes('mobile'),
    'Touch pointer lifetime',
  );
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole('region', { name: 'Local sector minimap' }),
  ).toBeVisible();
  const stick = page.getByRole('application', {
  ...
  // The objective panel was removed from the flight HUD by request; it lives in
  // the pause menu now, so nothing may overlap the joystick from above.
  await expect(
...
test('UX-T-103 overlay updates retain typing focus and settings can be cancelled', async ({
  page,
}) => {
  await page.goto('/');
  ...
  await expect(
    page.getByLabel('High contrast', { exact: true }),
  ).not.toBeChecked();
  await page.getByRole('button', { name: 'Cancel', exact: true }).click();
  await startCampaignFromHome(page);
  await page
